...
class ColorBinarizationNode(AniNodeInterface):
    def new(name,parent):
        ...
        export_group_tree = bpy.data.node_groups.new(name,'CompositorNodeTree')
        ...
        export_group.node_tree = bpy.data.node_groups[export_group_tree.name]
...
class BinarizationNode(AniNodeInterface):
    def new(name,parent):
        ...
        blk_binarize_node = BlackBinarizationNode.new(name="Black Binarization",parent=export_group_tree)
        blk_binarize_node.location = (-300,310)
        blk_binarize_node.hide = True

        red_binarize_node = ColorBinarizationNode.new(name="Red Binarization",parent=export_group_tree)
        red_binarize_node.location = (-300,230)
        red_binarize_node.hide = True

        green_binarize_node = ColorBinarizationNode.new(name="Green Binarization",parent=export_group_tree)
        green_binarize_node.location = (-300,150)
        green_binarize_node.hide = True

        blue_binarize_node = ColorBinarizationNode.new(name="Blue Binarization",parent=export_group_tree)
        blue_binarize_node.location = (-300,70)
        blue_binarize_node.hide = True

        blk_filter_node = BlackFilterNode.new(name="Black Filter",parent=export_group_tree)
        blk_filter_node.location = (-300,-70)
        blk_filter_node.hide = True

        #Red:   Hue = 0.000000
        red_filter_node = ColorFilterNode.new(name="Red Filter",parent=export_group_tree)
        red_filter_node.location = (-300,-150)
        red_filter_node.inputs['Hue'].default_value = 0.00
        red_filter_node.hide = True

        #Green: Hue = 0.333333
        green_filter_node = ColorFilterNode.new(name="Green Filter",parent=export_group_tree)
        green_filter_node.location = (-300,-230)
        green_filter_node.inputs['Hue'].default_value = 0.333333
        green_filter_node.hide = True

        #Blue:  Hue = 0.666667
        blue_filter_node = ColorFilterNode.new(name="Blue Filter",parent=export_group_tree)
...
class AniScannerNode(AniNodeInterface):
    def new(name,parent):
        export_group = parent.nodes.new('CompositorNodeGroup')
        # export_group.node_tree = bpy.data.node_groups[name]
        export_group_tree = bpy.data.node_groups.new(name,'CompositorNodeTree')
        ...
        binarize_node = BinarizationNode.new("Binarization",export_group_tree)
        ...
        paper_node = PaperOutputNode.new("Paper Output",export_group_tree)
